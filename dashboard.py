...
Address = {self.allowed_ip}'''
        ...
        # Add MTU only if it has a valid value
        if self.mtu and str(self.mtu).strip():
            peerConfiguration += f"\nMTU = {str(self.mtu)}"
...
Endpoint = {DashboardConfig.GetConfig("Peers", "remote_endpoint")[1]}:{self.configuration.ListenPort}'''
        ...
        # Add PersistentKeepalive only if it has a valid value
        if self.keepalive and str(self.keepalive).strip():
...
Address = {self.allowed_ip}'''
        ...
        # Add Amnezia-specific parameters
        peerConfiguration += f'''
    ...
    if authenticationRequired:
        ...
        if apiKey is not None and len(apiKey) > 0 and apiKeyEnabled:
            ...
            whiteList = [
                ...
                'getAvailableIPs', 'downloadPeer', 'downloadAllPeers'
            ]
