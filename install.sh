...
copy_application_files() {
    ...
    if [ ! -f "$INSTALL_DIR/db/users_schema.sql" ]; then
        print_error "users_schema.sql not found in db/ directory"
        exit 1
    fi
    ...
    if [ -f "$INSTALL_DIR/db/users_schema.sql" ]; then
        sqlite3 "$INSTALL_DIR/db/users.db" < "$INSTALL_DIR/db/users_schema.sql"
        print_success "Users schema initialized"
    fi
